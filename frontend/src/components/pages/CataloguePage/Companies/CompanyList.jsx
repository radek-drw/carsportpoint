...
const CompanyList = () => {
  const { companies, isLoading, setSelectedCompany, hoveredCompany, setHoveredCompany, mapRef } =
    useCompany();
  ...
  return (
    <div className="basis-1/2 md:pr-[1vw]">
      <ul className="rounded-lg shadow-lg">
        {isLoading
          ? [...Array(6)].map((_, index) => (
              <SkeletonTheme baseColor="#d1d5db" highlightColor="#f9fafb" key={index}>
                <li className="rounded-md px-4 py-2">
                  <Skeleton height={22} width="80%" duration={1} />
                  <Skeleton height={15} width="60%" style={{ marginTop: 6 }} duration={1} />
                </li>
              </SkeletonTheme>
            ))
